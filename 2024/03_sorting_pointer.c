...
void printValues(float *values, int size) {
    ...
    for (int i = 0; i < size; i++) {
        printf("%.3f", *(values + i)); // Pointer ile dizideki elemanlara erişim.
        if (i < size - 1) {
...
int findLastIndexOfValue(float *values, int size, float v) {
	...
	for (int i = size - 1; i >= 0; i--) {
		if (*(values + i) == v) {  // Pointer ile eleman karşılaştırma.
...
float maxElement(float *values, int size) {
    float maxVal = *values; // İlk elemanı en büyük kabul ederek başlıyoruz.
	
	// Tüm elemanları karşılaştırıp en büyüğünü buluyoruz.
    for (int i = 1; i < size; i++) {
        if (*(values + i) > maxVal) {  // Pointer ile eleman karşılaştırma.
...
void sortOnValue(float *values, int size) {
	...
	do {
        ...
        for (int i = 0; i < size - 1; i++) {
            ...
            if (*(values + i) > *(values + i + 1)) {  // Pointer ile karşılaştırma.
...
int main() {
    ...
    float *ptr_val = val; // Diziye pointer ile erişim sağlamak için pointer tanımladık.
    ...
    while (1) {
        ...
        switch (command) {
            ...
            case 'f': {  // 'f' komutu bir değeri bulur.
                ...
                int index = findLastIndexOfValue(ptr_val, 10, valueToFind);
                if (index != -1) {
            ...
            case 'r': {  // 'r' komutu dizideki bir elemanı değiştirmeye yarar.
                ...
                scanf("%f", &newValue);
                replaceElement(ptr_val, index, newValue);
                break;
            }

            case 's':  // 's' komutu diziyi sıralar.
                sortOnValue(ptr_val, 10);
                break;
